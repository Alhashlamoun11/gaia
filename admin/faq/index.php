...
/**
 * admin/faq/index.php
 * FAQ management (category, question, answer, sort_order, status).
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id > 0) {
        if ($action === 'toggle') {
            $st = $pdo->prepare('SELECT is_active FROM faq WHERE id=?'); $st->execute([$id]);
            $cur = (int)$st->fetchColumn();
            $pdo->prepare('UPDATE faq SET is_active=? WHERE id=?')->execute([$cur?0:1,$id]);
        ...
        header('Location: ' . admin_url('faq/index.php')); exit;
    }
}
...
$s  = AdminQuery::sort(['id','category','question','sort_order'], 'sort_order');

$where = ['1=1']; $params = [];
if ($q !== '') { $where[] = '(question LIKE :q OR answer LIKE :q OR category LIKE :q)'; $params[':q'] = '%'.$q.'%'; }
if ($st !== '') { $where[] = 'is_active = :st'; $params[':st'] = (int)$st; }
$wh = implode(' AND ', $where);

$c = $pdo->prepare("SELECT COUNT(*) FROM faq WHERE $wh"); $c->execute($params);
$total = (int)$c->fetchColumn();
$pages = max(1, (int)ceil($total / $pg['per_page']));
if ($pg['page'] > $pages) { $pg['page']=$pages; $pg['offset']=($pg['page']-1)*$pg['per_page']; }

$stmt = $pdo->prepare("SELECT * FROM faq WHERE $wh ORDER BY {$s['col']} {$s['dir']} LIMIT {$pg['per_page']} OFFSET {$pg['offset']}");
$stmt->execute($params);
$rows = $stmt->fetchAll();

$url_builder = fn($ov=[]) => '?' . http_build_query(array_merge($_GET, $ov));
$account_alerts = admin_flash_peek() ? [admin_flash()] : [];
$admin_page_title = t('admin.faq').' — GAIA TOURS &amp; TRAVEL';
$admin_topbar_title = t('admin.faq');
$admin_active = 'faq';
?>
...
<div class="admin-app">
  ...
  <div class="admin-main">
    ...
    <div class="admin-content">
      ...
      <div class="card">
        <div class="card-head">
          <h3><i class="fa-solid fa-circle-question"></i> <?= htmlspecialchars(t('admin.faq')) ?></h3>
          <a class="btn btn-sm" href="form.php"><?= htmlspecialchars(t('admin.add_faq','Add FAQ')) ?></a>
        </div>
          ...
          <table>
            ...
            <?php else: foreach ($rows as $r): ?>
              <tr>
                ...
                <td class="code"><?= htmlspecialchars($r['question']) ?></td>
                <td class="truncate"><?= htmlspecialchars(mb_substr($r['answer'], 0, 60)) ?></td>

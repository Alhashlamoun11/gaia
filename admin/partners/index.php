 ...
 * Partners management (name, logo/icon, website, order, status).
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id > 0) {
        if ($action === 'toggle') {
            $st = $pdo->prepare('SELECT is_active FROM partners WHERE id=?'); $st->execute([$id]);
            $cur = (int)$st->fetchColumn();
            $pdo->prepare('UPDATE partners SET is_active=? WHERE id=?')->execute([$cur?0:1,$id]);
        ...
        header('Location: ' . admin_url('partners/index.php')); exit;
    }
}
...
$s  = AdminQuery::sort(['id','name','sort_order'], 'sort_order');

$where = ['1=1']; $params = [];
if ($q !== '') { $where[] = '(name LIKE :q OR website LIKE :q)'; $params[':q'] = '%'.$q.'%'; }
if ($st !== '') { $where[] = 'is_active = :st'; $params[':st'] = (int)$st; }
$wh = implode(' AND ', $where);

$c = $pdo->prepare("SELECT COUNT(*) FROM partners WHERE $wh"); $c->execute($params);
$total = (int)$c->fetchColumn();
$pages = max(1, (int)ceil($total / $pg['per_page']));
if ($pg['page'] > $pages) { $pg['page']=$pages; $pg['offset']=($pg['page']-1)*$pg['per_page']; }

$stmt = $pdo->prepare("SELECT * FROM partners WHERE $wh ORDER BY {$s['col']} {$s['dir']} LIMIT {$pg['per_page']} OFFSET {$pg['offset']}");
$stmt->execute($params);
$rows = $stmt->fetchAll();

$url_builder = fn($ov=[]) => '?' . http_build_query(array_merge($_GET, $ov));
$account_alerts = admin_flash_peek() ? [admin_flash()] : [];
$admin_page_title = t('admin.partners').' — GAIA TOURS &amp; TRAVEL';
$admin_topbar_title = t('admin.partners');
...
<div class="admin-app">
  ...
  <div class="admin-main">
    ...
    <div class="admin-content">
      ...
      <div class="card">
        <div class="card-head">
          <h3><i class="fa-solid fa-handshake"></i> <?= htmlspecialchars(t('admin.partners')) ?></h3>
          <a class="btn btn-sm" href="form.php"><?= htmlspecialchars(t('admin.add_new')) ?></a>
        </div>
        ...
        <div class="table-wrap">
          <table>
            ...
            <?php else: foreach ($rows as $r): ?>
              <tr>
                <td><?php if (!empty($r['icon'])): ?><img class="thumb" src="<?= htmlspecialchars($r['icon']) ?>" alt=""><?php else: ?><span class="muted">—</span><?php endif; ?></td>
                <td class="code"><?= htmlspecialchars($r['name']) ?></td>
                <td><?= !empty($r['website'])?htmlspecialchars($r['website']):'—' ?></td>

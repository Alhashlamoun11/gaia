<?php
/**
 * admin/pages/form.php
 * CMS Pages create/edit.
 */
require_once __DIR__ . '/../_bootstrap.php';
require_admin();

$pdo = getPDO();
$id  = (int)($_GET['id'] ?? 0);
$editing = $id > 0;
$row = $editing ? CrudService::find('pages', $id) : null;
if ($editing && !$row) { http_response_code(404);
    $admin_page_title = t('admin.not_found','Not Found').' — GAIA TOURS & TRAVEL'; require __DIR__.'/../components/header.php';
    echo '<div class="admin-app"><div class="admin-main"><div class="admin-content"><div class="card"><div class="muted" style="text-align:center;padding:40px;">'.htmlspecialchars(t('admin.not_found_text','Not found.')).'</div></div></div></div></div>'; require __DIR__.'/../components/footer.php'; exit;
}

$errors = []; $alerts = [];
$old = $row ?: ['slug'=>'','title'=>'','content'=>'','lang'=>'en','sort_order'=>0,'is_active'=>1];

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!csrf_verify()) csrf_fail();
    $data = [
        'slug' => trim($_POST['slug'] ?? ''),
        'title' => trim($_POST['title'] ?? ''),
        'content' => trim($_POST['content'] ?? ''),
        'lang' => in_array($_POST['lang'] ?? 'en', ['en','ar'], true) ? $_POST['lang'] : 'en',
        'sort_order' => (int)($_POST['sort_order'] ?? 0),
        'is_active' => !empty($_POST['is_active']) ? 1 : 0,
    ];
    if ($data['slug']==='') $errors[]=t('admin.slug_req','Slug is required.');
    if ($data['title']==='') $errors[]=t('admin.title_req','Title is required.');

    if (!$errors) {
        if ($editing) { CrudService::update('pages',$id,$data); $alerts[]=['type'=>'success','msg'=>t('admin.saved','Saved successfully.')]; }
        else { $id=CrudService::insert('pages',$data); $editing=true; $alerts[]=['type'=>'success','msg'=>t('admin.created','Created successfully.')]; }
        $row=CrudService::find('pages',$id); $old=$row;
    }
}

$account_alerts = array_merge(array_map(fn($e)=>['type'=>'error','msg'=>$e], $errors), $alerts);
$admin_page_title = ($editing?t('admin.edit'):t('admin.create')).' — GAIA TOURS & TRAVEL';
$admin_topbar_title = ($editing&&$row)?$row['title']:t('admin.add_new');
$admin_active = 'pages';
$C = fn($v)=>htmlspecialchars((string)($v ?? ''));
?>
<?php require __DIR__.'/../components/header.php'; ?>
<div class="admin-app">
  <?php require __DIR__.'/../components/sidebar.php'; ?>
  <div class="admin-main">
    <?php require __DIR__.'/../components/topbar.php'; ?>
    <div class="admin-content">
      <?php require __DIR__.'/../components/alert.php'; ?>
      <div class="card">
        <div class="card-head">
          <h3><i class="fa-solid fa-file-lines"></i> <?= $C(t('admin.page_info','Page Information')) ?></h3>
          <a class="btn btn-ghost btn-sm" href="index.php"><?= $C(t('admin.back_to_list')) ?></a>
        </div>
        <form method="post" action="form.php<?= $editing?'?id='.(int)$id:'' ?>" novalidate>
          <?= csrf_field() ?>
          <div class="grid-2">
            <div class="field"><label><?= $C(t('admin.slug')) ?> *</label><input type="text" name="slug" value="<?= $C($old['slug']) ?>" required placeholder="about"></div>
            <div class="field"><label><?= $C(t('admin.title')) ?> *</label><input type="text" name="title" value="<?= $C($old['title']) ?>" required></div>
          </div>
          <div class="grid-2">
            <div class="field"><label><?= $C(t('admin.lang')) ?></label>
              <select name="lang">
                <option value="en" <?= $old['lang']==='en'?'selected':'' ?>>English</option>
                <option value="ar" <?= $old['lang']==='ar'?'selected':'' ?>>Arabic</option>
              </select>
            </div>
            <div class="field"><label><?= $C(t('admin.sort_order')) ?></label><input type="number" name="sort_order" value="<?= (int)($old['sort_order']??0) ?>"></div>
          </div>
          <div class="field"><label><?= $C(t('admin.content','Content')) ?></label><textarea name="content" rows="12"><?= $C($old['content']) ?></textarea></div>
          <div class="field"><label><?= $C(t('admin.status')) ?></label><input type="checkbox" name="is_active" value="1" <?= (int)$old['is_active']?'checked':'' ?>> <?= $C(t('admin.active')) ?></div>
          <div class="form-actions">
            <button type="submit" class="btn"><?= $C(t('admin.save')) ?></button>
            <a class="btn btn-ghost" href="index.php"><?= $C(t('admin.cancel')) ?></a>
          </div>
        </form>
      </div>
    </div>
  </div>
</div>
<?php require __DIR__.'/../components/footer.php'; ?>
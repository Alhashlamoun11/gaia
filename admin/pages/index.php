 ...
 * CMS Pages CRUD (About, Contact, Privacy, Terms, Cookies).
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id > 0) {
        if ($action === 'toggle') {
            $st = $pdo->prepare('SELECT is_active FROM pages WHERE id=?'); $st->execute([$id]);
            $cur = (int)$st->fetchColumn();
            $pdo->prepare('UPDATE pages SET is_active=? WHERE id=?')->execute([$cur?0:1,$id]);
...
$q  = AdminQuery::search();
$s  = AdminQuery::sort(['id','slug','title','sort_order'], 'slug');

$where = ['1=1']; $params = [];
if ($q !== '') { $where[] = '(slug LIKE :q OR title LIKE :q OR content LIKE :q)'; $params[':q'] = '%'.$q.'%'; }
$wh = implode(' AND ', $where);

$c = $pdo->prepare("SELECT COUNT(*) FROM pages WHERE $wh"); $c->execute($params);
$total = (int)$c->fetchColumn();
$pages = max(1, (int)ceil($total / $pg['per_page']));
if ($pg['page'] > $pages) { $pg['page']=$pages; $pg['offset']=($pg['page']-1)*$pg['per_page']; }

$stmt = $pdo->prepare("SELECT * FROM pages WHERE $wh ORDER BY {$s['col']} {$s['dir']} LIMIT {$pg['per_page']} OFFSET {$pg['offset']}");
$stmt->execute($params);
$rows = $stmt->fetchAll();

$url_builder = fn($ov=[]) => '?' . http_build_query(array_merge($_GET, $ov));
$account_alerts = admin_flash_peek() ? [admin_flash()] : [];
$admin_page_title = t('admin.pages').' — GAIA TOURS & TRAVEL';
$admin_topbar_title = t('admin.pages');
$admin_active = 'pages';
...
<div class="admin-app">
  ...
  <div class="admin-main">
    ...
    <div class="admin-content">
      ...
      <div class="card">
        <div class="card-head">
          <h3><i class="fa-solid fa-file-lines"></i> <?= $C(t('admin.pages')) ?></h3>

 ...
 * Services management (icon, title, description, order, status).
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id > 0) {
        if ($action === 'toggle') {
            $st = $pdo->prepare('SELECT is_active FROM services WHERE id=?'); $st->execute([$id]);
            $cur = (int)$st->fetchColumn();
            $pdo->prepare('UPDATE services SET is_active=? WHERE id=?')->execute([$cur?0:1,$id]);
            admin_flash(t('admin.status_updated','Status updated.'), 'success');
        } elseif ($action === 'delete') {
            $pdo->prepare('DELETE FROM services WHERE id=?')->execute([$id]);
            admin_flash(t('admin.deleted','Deleted.'), 'success');
        }
        header('Location: ' . admin_url('services/index.php')); exit;
    }
}
...
$s  = AdminQuery::sort(['id','title','sort_order'], 'sort_order');
...
$c = $pdo->prepare("SELECT COUNT(*) FROM services WHERE $wh"); $c->execute($params);
$total = (int)$c->fetchColumn();
$pages = max(1, (int)ceil($total / $pg['per_page']));
if ($pg['page'] > $pages) { $pg['page']=$pages; $pg['offset']=($pg['page']-1)*$pg['per_page']; }

$stmt = $pdo->prepare("SELECT * FROM services WHERE $wh ORDER BY {$s['col']} {$s['dir']} LIMIT {$pg['per_page']} OFFSET {$pg['offset']}");
$stmt->execute($params);
$rows = $stmt->fetchAll();

$url_builder = fn($ov=[]) => '?' . http_build_query(array_merge($_GET, $ov));
$account_alerts = admin_flash_peek() ? [admin_flash()] : [];
$admin_page_title = t('admin.services').' — GAIA TOURS &amp; TRAVEL';
$admin_topbar_title = t('admin.services');
...
<div class="admin-app">
  ...
  <div class="admin-main">
    ...
    <div class="admin-content">
      ...
      <div class="card">
        <div class="card-head">
          <h3><i class="fa-solid fa-bell-concierge"></i> <?= htmlspecialchars(t('admin.services')) ?></h3>
        ...
        <div class="table-wrap">
          <table>
            ...
            <?php else: foreach ($rows as $r): ?>
              <tr>
                <td><i class="fa-solid <?= htmlspecialchars($r['icon']) ?>"></i></td>
                <td class="code"><?= htmlspecialchars($r['title']) ?></td>
                <td class="truncate"><?= htmlspecialchars($r['description']) ?></td>
